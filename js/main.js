(function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (!allValid) {
      setStatus(
        "Merci de corriger les champs en rouge avant d'envoyer.",
        "error",
      );
      return;
    }
    ...
    if (form.action.includes("YOUR_FORM_ID")) {
      setStatus(
        "⚠️ Configuration requise : remplace YOUR_FORM_ID par ton vrai ID Formspree dans le code.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setStatus(
          "✅ Message envoyé ! Je reviens vers toi très vite.",
          "success",
        );
        form.reset();
      } else {
        setStatus(
          "Oups, une erreur est survenue. Réessaie ou écris-moi directement par email.",
          "error",
        );
      }
    } catch (err) {
...
(function () {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && curtain.classList.contains("is-open"))
      closeMenu();
  });
})();
